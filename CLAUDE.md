...
- 影片深連結:`vurl`(站台 siteUrl) `v`(vod_id) `src`(來源索引) `ep`(集索引)
- 瀏覽清單:`site`(站名) `cat`(分類) `page`(資料頁) `q`(關鍵字) `sites`(多站搜尋,站名逗號清單)

影片深連結的站台用 **siteUrl**(跟觀看歷史 / 收藏同一套識別):寫入直接吃播放資訊(`videoInfo.siteUrl`)、不反查站台,所以一般瀏覽 / 多來源 / 分享 / **歷史續看**都能可靠寫出網址;還原端(`openVideoFromUrl`)直接拿 url 打 API,**不要求該站在本地清單**,跨裝置 / 跨 app(kazi)寫的歷史也能分享、還原。代價:站台 API 網址換了舊連結會失效(但 url 比站名穩、且跟歷史同一套識別)。瀏覽清單仍用「站名」入網址(較好讀)。
